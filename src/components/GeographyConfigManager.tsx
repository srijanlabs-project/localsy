  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h4 className="text-sm font-bold text-slate-950">Geo-Boundary Coverage</h4>
            <p className="mt-1 text-[11px] text-slate-500">
              Derived locality boundaries are built from listing GPS coordinates so ops can spot which localities are map-ready and which still need geo enrichment.
            </p>
          </div>
          <button
            type="button"
            onClick={() => {
              ...
              fetch('/api/admin/geography/boundaries')
                ...
                .then(({ response, payload }) => {
                  if (!response.ok || !payload?.ok) {
                    throw new Error(payload?.error || 'Failed to refresh locality boundaries.');
                  }
                  setBoundaries(Array.isArray(payload.boundaries) ? payload.boundaries : []);
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-3">
            <div className="text-[10px] uppercase tracking-wide text-emerald-700">Boundary Ready</div>
            <div className="mt-1 text-2xl font-extrabold text-emerald-900">
              {boundaries.filter((entry) => entry.status === 'derived').length}
            </div>
          </div>
          <div className="rounded-xl border border-amber-100 bg-amber-50 p-3">
            <div className="text-[10px] uppercase tracking-wide text-amber-700">Missing Coverage</div>
            <div className="mt-1 text-2xl font-extrabold text-amber-900">
              {boundaries.filter((entry) => entry.status !== 'derived').length}
            </div>
          </div>
          <div className="rounded-xl border border-sky-100 bg-sky-50 p-3">
            <div className="text-[10px] uppercase tracking-wide text-sky-700">GPS Anchor Points</div>
            <div className="mt-1 text-2xl font-extrabold text-sky-900">
              {boundaries.reduce((total, entry) => total + Number(entry.pointCount || 0), 0)}
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left text-[11px] text-slate-600">
            <thead className="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-400">
              <tr>
                ...
                <th className="px-3 py-2">GPS Points</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {boundaries.map((entry) => (
                <tr key={entry.localityId}>
                  <td className="px-3 py-2 font-semibold text-slate-900">{entry.localityName}</td>
                  <td className="px-3 py-2">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold ${
                      entry.status === 'derived' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                    }`}>
                      {entry.status}
                    </span>
                  </td>
                  <td className="px-3 py-2">{entry.pointCount || 0}</td>
                  <td className="px-3 py-2 font-mono">
                    {entry.center ? `${entry.center.lat}, ${entry.center.lng}` : 'Not available'}
                  </td>
                  <td className="px-3 py-2 font-mono">
                    {entry.bounds
                      ? `${entry.bounds.minLat} to ${entry.bounds.maxLat} / ${entry.bounds.minLng} to ${entry.bounds.maxLng}`
                      : 'Not available'}
                  </td>
                </tr>
              ))}
              {!isLoadingBoundaries && boundaries.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-3 py-6 text-center text-slate-400">
                    No boundary coverage available yet.
                  </td>
                </tr>
              )}
              {isLoadingBoundaries && (
